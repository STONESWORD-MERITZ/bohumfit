    ...
    <div className="flex min-h-dvh items-center justify-center overflow-x-clip bg-canvas px-4">
      <div className="w-full max-w-sm min-w-0">
        <div className="mb-8 text-center">
          ...
          <p className="mt-3 text-body text-ink-soft">등록 휴대폰으로 비밀번호를 재설정합니다</p>
        </div>
        ...
        <section className="rounded-card border border-line bg-white p-6">
          ...
          {step === "code" && (
            <form onSubmit={verifyCode} className="mt-5 space-y-3">
              ...
              <button type="button" onClick={() => setStep("phone")} className="w-full text-xs font-semibold text-ink-soft hover:underline">
                번호 다시 입력
              </button>
            </form>
          )}
