  ...
  return (
    <div className={`rounded-[10px] border border-line bg-ink-50 p-4 ${className}`}>
      ...
      {/* BOHUMFIT-265(보정 · Human 확정 문구): 개인정보처리방침과 **3층 보관 기간**을 일치시킨다.
          ...
          ★저장하지 않는 것은 **업로드 자료 원본**이고, 분석 결과는 90일/7일 서버 보관 + 기기 임시 보관이 정확하다.
          ★보조 문구 하한 15px 규칙 준수(고객에게 보여주는 고지문).
          BOHUMFIT-279: 265 오프라인 캐시 A안 **철회**(Human 확정) — 배선되지 않은 "최근 5건·24시간"을
            실동작(**세션 저장소 10분**, 277 기준)으로 교체한다. 방침 4항과 같은 사실을 말한다. */}
      <p className="mt-2 text-[15px] leading-[1.55] text-ink-soft break-keep">
        ...
        <b className="font-semibold text-ink-900">7일간</b> 서버에 보관되며, 다시 보기용으로{" "}
        <b className="font-semibold text-ink-900">이 기기에 10분간 임시 보관</b>됩니다(로그아웃·계정 전환 시 즉시 삭제). 출력물은
