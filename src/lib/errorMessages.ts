...
const RULES: Rule[] = [
  ...
  // ★BOHUMFIT-284(283 F-7): 쪽수·압축 해제 크기 상한. 크기(MB) 문구와 조각이 겹치지 않아
  //   순서 의존이 없지만, 같은 "업로드 전 단계"라 크기 규칙 옆에 둔다.
  {
    match: ["쪽수가 너무 많", "쪽 초과"],
    message: "PDF 쪽수가 너무 많아요. 발급 기간을 나눠 받은 뒤 다시 올려 주세요.",
  },
  {
    match: ["내용이 지나치게 커서"],
    message: "PDF를 열지 못했어요. 발급 기간을 나눠 받은 뒤 다시 올려 주세요.",
  },
  {
  ...
  // ── 레이트리밋(429) ──────────────────────────────────────────────────
  //   ★BOHUMFIT-284: 서버는 "요청이 너무 잦습니다"라고 정확히 알려주는데 **사전에 규칙이 없어**
  //     화면에는 폴백("요청을 처리하지 못했어요")이 떴다. 284가 limiter를 4종에 추가해 429가
  //     실제로 늘어나므로 함께 채운다. ★"잠시 후"만으로는 부족해 **얼마나 기다릴지**를 준다.
  {
    match: ["요청이 너무 잦"],
    message: "요청이 너무 잦아요. 1분 정도 기다린 뒤 다시 시도해 주세요.",
  },
  // ── 서버·시간 초과 ───────────────────────────────────────────────────
  {
