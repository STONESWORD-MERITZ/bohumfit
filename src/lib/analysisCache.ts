// ★BOHUMFIT-279 — 265 오프라인 캐시 A안은 **철회**됐다(Human 확정, 2026-08-08).
//   `saveAnalysis`·`listAnalyses`·`getAnalysis`는 **제품 코드에서 호출되지 않는다**(275 B-F4 실측·279 재확인).
//   ★파일을 지우지 않는 이유: 재개 시 265 설계(스토어 분리·5건·24h·user-bound)를 처음부터 다시 해야 한다.
//   ★단 `clearAnalysisCache()`는 **계속 호출된다** — `AuthContext`의 단일 삭제 지점에서 과거 잔여
//     레코드를 지우는 역할이고, 277이 같은 지점에 넣은 `clearSessionResult()`와 함께 유지된다.
//   재개 조건: ①오프라인 열람 수요 확인 ②user-bound 저장·목록·복원 동선 배선
//             ③모든 기기 저장소를 하나의 삭제 계약에 포함 ④방침·동의문 문구 동시 갱신.
