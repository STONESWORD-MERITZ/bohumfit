...
```text
750 passed, 8 skipped
```

(BOHUMFIT-255 실측·2026-07-29 갱신 — 직전 표기 684/8은 242 시점 값으로 낡아 있었다.)
...
(BOHUMFIT-255 실측·2026-07-29 갱신 — 직전 표기 79는 242 시점 값.)
...
- 스모크 실행은 **사용자 동선(엔드포인트 `POST /coverage/export/excel`) 경유**로 한다(249 원칙 —
  "산출물 검증은 사용자 동선 기준"). 별도 상비 스크립트는 없으며, 태스크별 검증 스크립트는
  스크래치에 생성하고 실행 후 삭제한다(실데이터 산출물 저장 금지).
