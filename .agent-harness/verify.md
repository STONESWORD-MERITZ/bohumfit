...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 표준(계약별 매트릭스) | 상품별 가입현황 존재 | `보장분석\비교분석표\이*숙-INPUT.pdf` | 15 | 59/29 | 614,860,000 | 681,312 (531,312) | 0 | 0 |
| overview(합계-only) | 239 fallback(매트릭스 부재) | `보장분석\비교분석표\라*실INPUT.pdf` | 15 | 58/41 | 1,467,790,000 | 4,675,189 (4,675,189) | 25 | 2 |
...
- ★**BOHUMFIT-296/296b Human Q6 재승인 반영**: 산출물 스키마를 49→52행으로 바꾸고 N대수술비를
  계약별 복수 담보 `sum→max`로 정정했다. 표준 담보행 57→59(총액 불변), overview 총액
  1,501,690,000→1,500,690,000(−1,000,000). 추가 정본 C −11,700,000·D −4,000,000도 같은
  N대수술비 정정분이며, 3대비급여실손 9,000,000·6주미만 10,000,000 이관은 값 보존이다.
  292의 `2열 병기 (좌 | 우)` 헤더 행은 제거돼 `track_row_of()`가 다시 고정행 좌표를 쓴다.
- ★**BOHUMFIT-301 Human Q6 승인 반영**: [전] 종수술 개별행을 질병/상해 tier 5행으로 옮기고,
  개별행이 있으면 요약행 238 환산을 폐기하며 6종 이상을 무시한다. 표준 정본은 완전 불변,
  overview는 담보행 `59→58`·enrolled `42→41`·총액 `1,500,690,000→1,467,790,000`
  (−32,900,000: 요약행 환산 27,500,000 + 6~8종 오환산 5,400,000 제거)·경고 `1→2`다.
  추가 정본 C는 담보행 `56→55`이지만 bare 21,500,000이 질병 tier로 위치만 이동해 총액은 불변이다.

### ★스키마 정본 (BOHUMFIT-293 · 층위 2 정리 완료)

- **52행 V2가 유일한 산출물 스키마다.** `KB_COVERAGES_V2`(52) · `GROUP12_V2`(11) · `PAYOUT_CASCADE_V2`(17체인)로
...
- **서식 회귀**는 `tests/test_format_regression_293.py`가 실문서 3건으로 막는다 — 차액 색상(261)·합계 강조·
  Q2 메모·Q5 메모·L 접두·2열 정합(296 헤더 제거)·브랜드 색(빨강 0)·인쇄 설정·PDF 고령 가독성·비고 보존.
  ★291에서 261 차액 색상이 조용히 빠진 유형을 겨냥한다(뮤테이션 4종으로 검출력 확인 완료).

### ★뮤테이션 검증의 범위 — "그 지점"에 심어라 (BOHUMFIT-295b 선례)

회귀 수정 태스크는 결함이 **있던 그 지점**에 뮤테이션을 심어 테스트의 검출력을 실증한다. 이때 흔한 함정:
**추출한 헬퍼의 로직 단위 테스트는 배선 회귀를 검출하지 못한다.**
- BOHUMFIT-295 선례: `CoverageRemodel`의 [후] 표시가 비대칭이던 것을 헬퍼(`displayStageTotals`)로 대칭화하고
  헬퍼를 단위 테스트했으나, **컴포넌트를 종전 비대칭 구현으로 되돌려도 `npm test` 410 passed로 통과**했다
  (헬퍼는 맞지만 컴포넌트가 그것을 쓰는지는 미검증). Codex 반려.
- 규칙: 결함이 "컴포넌트 배선"이면 **컴포넌트를 실제 렌더해 표시 결과를 단언**한다(헬퍼 단위 테스트로 대체 불가).
  뮤테이션 목록에 **원래 결함 형태(예: 종전 useMemo 복원) 자체**를 반드시 포함하고, 그 뮤테이션이 실패하는지 본다.
- `CoverageRemodelWiring295.test.tsx`가 그 렌더 계약이다(뮤테이션 3종 — 비대칭 useMemo 복원·헬퍼 무조건 재산출·
  stale 판정 제거 — 전부 실패 실증).
...
| BOHUMFIT-295/295b | **없음** — 프런트 표시 계층만(제안서 없음 [후]==[전] 복원). 백엔드 값 계층 diff 0 · 산출물 해시 12종 동일 | — |
| BOHUMFIT-296/296b | 52행 전환 — 표준 담보행 **57→59**(총액 불변) · overview 총액 **−1,000,000** · 추가 정본 C **−11,700,000**·D **−4,000,000**(전부 N대수술비 sum→max) | Human Q6 재승인 |
| BOHUMFIT-301 | [전] 종수술 tier 라우팅 — 표준 불변 · overview 담보행 **59→58**·총액 **−32,900,000**(요약행 환산 27.5M + 6~8종 오환산 5.4M 제거) · 추가 정본 C 담보행 **56→55**·총액 불변 | Human Q6 승인 |
