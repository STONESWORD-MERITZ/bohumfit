# BOHUMFIT-HARNESS-CODEX-ONLY: Codex 단독 하네스 문서 정리

## 목적

기존 Claude/Codex → Codex 검증·푸시 흐름을 정리하고, 이후 BOHUMFIT 작업을 Codex 단독 하네스 기준으로 진행하도록 문서를 맞춘다.
...
- `git diff --check`
- `rg -n "Cowork|Claude \\(|Claude/Cowork" .agent-harness/tasks --glob "!BOHUMFIT-HARNESS-CODEX-ONLY.md"` (no matches expected)
...
- `rg -n "Next: Codex 검증 \\+ 푸시|검증 \\+ 푸시" .agent-harness/tasks --glob "!BOHUMFIT-HARNESS-CODEX-ONLY.md"` (no matches expected)
