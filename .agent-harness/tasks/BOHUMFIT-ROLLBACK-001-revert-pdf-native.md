# BOHUMFIT-ROLLBACK-001: PDF 네이티브 첨부 롤백 + 텍스트 필터링 강화

## 목적
BOHUMFIT-007 ~ BUG-005(PDF 네이티브 첨부 시도)를 롤백하고
텍스트 방식으로 복귀한다. 동시에 텍스트 필터링을 강화해
318페이지 PDF에서도 유효 진료 데이터를 최대한 커버한다.

## 롤백 대상 커밋
- 7769951 BOHUMFIT-007 PDF 네이티브 첨부
...
- backend/tests/test_pdf_native.py 삭제 (BOHUMFIT-007 신규)
...
## 진단 메모 (1단계 확인)
- git log 확인: d60ccba7 = BUG-002, 그 이후 BOHUMFIT-007/BUG-003/4/5 가 PDF 네이티브 첨부 4단계 커밋.
...
- pdf_parser.py 도 BUG-002 시점 상태로 복원 (BOHUMFIT-007 의 `pdf_bytes` 필드 추가 제거).
- test_pdf_native.py 는 BOHUMFIT-007 (7769951) 에서 추가됐으므로 삭제.
