...
# ── 8) 파서: 2줄·총진료비(max=총액)·발급기간 ─────────────────────────────
def test_parse_nhis_text_cost_and_period():
    sample = (
        "건강보험 요양급여내역\n발급기간 2017.01.01 ~ 2017.12.31\n"
        "2017.03.10 1 행복의원 02-111-2222\n1\n입원 5 추간판장애 M51 1,200,000 900,000 300,000\n"
        "2017.06.20 2 OO의원 031-333-4444\n2\n외래 1 결막염 H10 90,000 60,000 30,000\n"
    )
    recs = parse_nhis_text(sample, "익명.pdf")
    assert _extract_nhis_issue_period(sample) == "2017.01.01 ~ 2017.12.31"
    assert len(recs) == 2
    assert recs[0]["총진료비"] == "1200000" and recs[0]["_issue_period"] == "2017.01.01 ~ 2017.12.31"
    assert recs[1]["총진료비"] == "90000"
