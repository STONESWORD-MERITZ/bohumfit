...
def _build_system_prompt(
    ...
    step2_tag_rules = (
        ...
        "★ BOHUMFIT-038: AI(flagged_items)는 Q2(1년 이내 추가검사·재검사 의심)만 부여한다.\n"
        "  Q1·Q3·Q4·Q5(진단·입원·수술·통원·투약·중대질병)는 결정론 룰/공단 자료로 처리하므로 AI는 q를 부여하지 않는다."
    )
...
▶ 만성질환 30일이상 투약 판단:
  ...
  - 단, 3개월 이내에만 처방 기록이 있고 이전 기록 없음 → Q1 해당 가능"""
        json_duty_q_values = "Q2 (BOHUMFIT-038: AI 보조판단은 Q2 추가검사·재검사 의심에만. Q1/Q3/Q4/Q5·입원·수술·중대질병은 결정론 처리하므로 AI는 부여 금지)"
        json_hit_fields = """\
