...
def _norm_field_name(value: str | None) -> str:
    """Normalize PDF table headers so OCR spacing variants still map to the same field."""
    if value is None:
        return ""
...
def get_diagnosis_code(row) -> str:
    """Extract KCD code only from diagnosis columns, never from detail/pharma billing code columns."""
...
def get_diagnosis_name(row) -> str:
    """Extract disease name from basic diagnosis columns only."""
