...
def analyze_kb_coverage(pdf_bytes: bytes, jong_table: dict | None = None) -> dict:
    """KB 신정원 보장분석 제안서 PDF 바이트 → 리모델링 [전]/[최종] 데이터.

    비-KB/손상 PDF는 KBFormatError(ValueError) 발생 → 라우터에서 400.
    jong_table: BOHUMFIT-238 종수술 환산표(DB 로드본) — None이면 내장 기본표 fallback.
    ※ PII: pdf_bytes/파싱 중간물은 서버에 저장하지 않는다(호출부에서 즉시 폐기).
    """
    raw = parse_document(pdf_bytes, jong_table)
