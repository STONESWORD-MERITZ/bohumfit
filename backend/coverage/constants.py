...
EXTRA_PATTERNS: tuple[tuple[re.Pattern[str], str, str, bool], ...] = (
    ...
    #   라인(정본C [전] #24 특정암 100만·#25 특정암제외 500만 실측). classify_extra가 종별(일반/전립선/
    ...
    #   (정본C [전] #37·#39, 정본D #42·#43 실측). 중입자와 같은 target_included 차감 대상.
...
def davinci_label(compact_name: str) -> str:
    ...
    실측 패턴(정본C [전]): `다빈치로봇암수술비(연간1회한,특정암)` / `(연간1회한,특정암제외)`.
    """
...
KB_COVERAGES_V2: tuple[CoverageRowV2, ...] = (
    ...
    # BOHUMFIT-290: 별칭 = 파서 extras 라벨(288 실측 — 정본 2건에 실재: 정본A 2,000만·정본B 200만).
