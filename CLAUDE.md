...
이 파일은 기존 이름을 유지하지만, 현재 BOHUMFIT는 **Claude Chat→Claude Code→Codex 3역할 하네스 방식**으로 진행한다(BOHUMFIT-224로 Cowork 퇴역, 순서 불변).
Claude Chat은 프롬프트 작성자(레포 접근 없음·위험도 판단 책임), Claude Code는 Windows 로컬 구현+1차 검증 담당(tsc/lint/build/pytest 직접 실행·신뢰, git 읽기 허용·쓰기 기본 금지), Codex는 2차 검증·커밋·push·배포 확인 담당(직접 수정은 한 줄급만, 그 이상 반려)이다.
`AGENTS.md`가 최상위 운영 규칙이고(루트 게이트·위험도 운영 규칙·커밋 전 검증 계약 포함), `.agent-harness/WORKFLOW.md`가 새 채팅 기반 업무 처리 계약이다. 이 파일은 프로젝트 성격·사용자 선호·코드베이스 관례를 보완한다.

## 작업 시작 시 필수 순서
0. 루트 게이트: `pwd`=`C:\Users\18_rk\BOHUMFIT`, remote=`STONESWORD-MERITZ/bohumfit`, 리트머스 파일(`.agent-harness/tasks/BOHUMFIT-219-shared-rls-migration-alignment.md`) 존재 확인. 불일치 시 즉시 중단·Human 보고. 유사 폴더(바탕화면 사본·구버전)에서 작업 금지.
1. `AGENTS.md`를 먼저 읽는다.
2. `.agent-harness/handoff.md`의 최신 항목으로 현재 상태를 확인한다.
3. `.agent-harness/WORKFLOW.md`를 읽고 이번 요청이 Claude Chat / Claude Code / Codex 중 어느 단계인지 판단한다.
4. 지정된 `.agent-harness/tasks/{태스크ID}.md`를 읽거나, 문서 정리/긴급 수정이면 필요한 범위만 새 태스크로 기록한다.
5. `.agent-harness/locks.md`로 active lock 충돌이 없는지 확인하고, 편집 범위에 본인(Claude Code 또는 Codex) 잠금을 추가한다.
6. `git status --short -uall`을 확인한다(Claude Code는 git 읽기까지만, 쓰기는 기본 금지).
7. 태스크 파일 또는 `.agent-harness/verify.md`의 검증 명령을 실행한다(Claude Code=Windows 로컬 1차, Codex=2차 권위).
8. 완료 후 `handoff.md` 상단에 변경/검증/이슈/다음 행동을 표준 포맷으로 기록한다.
9. `locks.md`에서 본인 잠금을 해제한다.
10. 사용자가 publish를 요청했거나 태스크 완료 조건에 push가 있으면, 검증 통과 후 태스크 범위 파일만 stage → commit → push 한다. 주체는 기본 Codex, 저위험 태스크에서 Chat이 "Code 커밋 허용"을 명시한 경우에만 Claude Code.
...
- handoff의 Next는 `Claude Chat`, `Claude Code`, `Codex`, `Human` 중 실제 다음 담당자를 명확히 쓴다.
- 새 작업 지시는 Claude Chat→Claude Code→Codex 3역할 기준으로 해석한다. Claude Chat은 취지 중심 작업 패킷을 만들고 위험도를 판단·명시하며, Claude Code는 Windows 로컬에서 구현·1차 검증을 직접 실행하고, Codex는 2차 검증·커밋·푸시·배포 확인을 담당한다. 과거 handoff/task/locks의 Cowork 표기는 역사 기록이다.
- STEP 0 실측 없이 수정하지 않는다. 최소 수정 원칙 — 스펙 밖 발견은 기록만 하고 Human이 결정한다.
- 이상 신호(빌드 산출물 급감·핵심 구조 "없음" 보고·파일 절단 징후·과거 커밋 회귀) 감지 시 즉시 중단·보고한다.
...
- [퇴역] Cowork 이력: 마운트 truncation/git 손상은 퇴역한 Cowork 샌드박스 환경의 제약이었다(`.agent-harness/ENV-MOUNT-NOTES.md` 참조 — 과거 기록 해석용). 현행 Claude Code는 Windows 로컬에서 직접 실행하므로 이 제약이 없고, 권위 커밋은 Codex가 담당한다.
...
- 백엔드: `cd backend && python -m pytest -q`
  - 현재 기준선(BOHUMFIT-295/295b Codex 2차 실측 · 2026-08-18): `1058 passed, 8 skipped`
- 프런트 테스트: `npm test` — 현재 기준선 `412 passed / 42 files`(BOHUMFIT-295/295b Codex 2차 실측·2026-08-18, 라우트 스모크 18건 포함)
...
- 빌드: `npm run build && npm run build:verify` — ★기준선 정정(BOHUMFIT-248 · 2026-07-26):
  ...
  ★껍데기 크기는 **약 343 kB로 코드 증가에 따라 변동**한다(271에서 +477 B 실측) — 특정 바이트
  수치와의 일치·불일치를 판정 근거로 쓰지 말 것(판정은 build:verify 스크립트가 한다).
